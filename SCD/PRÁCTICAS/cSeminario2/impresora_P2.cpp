...
constexpr int 
    num_productores = 5,    // nº de productores
...
class ProdConsNSU : public HoareMonitor
{
 ...
 int                        // variables permanentes
   ...
   primera_ocupada,
   mult_5,             // Indica los multiplos de 5 añadidos desde la anterior llamada
   total_mult_5;              // Indica los multiplos de 5 totales añadidos

 CondVar ocupadas, libres, impresora;
...
/*
   ...
   ocupadas.notify_one();
*/
    if ( primera_libre == (primera_ocupada-1+num_celdas_total)%num_celdas_total )
      libres.wait();
    assert( primera_libre < num_celdas_total );
    buffer[primera_libre] = valor;
    primera_libre++;
...
bool ProdConsNSU::metodo_impresora()
{
    if(mult_5 == 0)
        impresora.wait();

    bool devuelve = !(total_mult_5 == num_items/5);
    if(devuelve){
        cout << "\nEl Nº NUEVO de mult(5) insertados es: ___   " << mult_5 << endl;
        mult_5 = 0;    
    }
    return devuelve;
}
...
void funcion_hebra_impresora(MRef <ProdConsNSU> monitor)
{
    bool seguir = true;
   while (seguir)
   {
      seguir = monitor->metodo_impresora();
   }
}
// -----------------------------------------------------------------------------
