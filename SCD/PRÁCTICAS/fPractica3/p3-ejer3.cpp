...
void funcion_filosofos( int id )
{
  ...
  while ( true )
  {
    ...
    MPI_Send(&propina,1,MPI_INT,mesa, tag, MPI_COMM_WORLD);                 //ENVIO DE LA PROPINA DE FORMA ASÍNCRONA Y SEGURA 
...
void funcion_camarero(int id){
   int sentados = 0, tag, valor, total_propinas = 0, flag;
   MPI_Status estado;
    
   while(true){
       MPI_Iprobe(MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &flag, &estado);
      if(flag > 0){
          if(estado.MPI_TAG == tag_propina_5){
              MPI_Recv(&valor,1,MPI_INT, MPI_ANY_SOURCE, tag ,MPI_COMM_WORLD, &estado);
          }
          else if(estado.MPI_TAG == tag_propina_4 || estado.MPI_TAG == tag_propina_3){
              MPI_Recv(&valor,1,MPI_INT, MPI_ANY_SOURCE, tag ,MPI_COMM_WORLD, &estado);
          }
      }
      ...
      if(tag == tag_levantarse || tag == tag_sentarse)
        MPI_Recv(&valor,1,MPI_INT, MPI_ANY_SOURCE, tag ,MPI_COMM_WORLD, &estado);
