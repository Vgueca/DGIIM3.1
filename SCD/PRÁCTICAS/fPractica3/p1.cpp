...
const int num_filosofos = 7,           // número de filósofos
    ...
    num_cocineros = 2,
          num_procesos =
              num_filo_ten + 1 + num_cocineros,  // número de procesos total
    id_camarero = num_filo_ten, id_cocinero_uno = num_filo_ten + 1,
          id_cocinero_dos = num_filo_ten + 2;

const int etiq_reservar_tenedor = 1, etiq_soltar_tenedor = 2,
          etiq_reservar_sitio = 3, etiq_soltar_sitio = 4, etiq_cocinero = 5;
...
void funcion_filosofos(int id) {
    ...
    while (true) {
        ...
        // SELECCIONAMOS EL CAMARERO
        id_cocinero = aleatorio<id_cocinero_uno, id_cocinero_dos>();
        MPI_Ssend(&id, 1, MPI_INT, id_cocinero, etiq_cocinero, MPI_COMM_WORLD);
...
void funcion_cocinero(int orden) {
    MPI_Status estado;
    int valor, esperando, id_filosofo, id_aceptable, min = 0, max = 0, j;
    int cuenta[num_filosofos] = {0, 0, 0, 0, 0, 0, 0};
    bool fil_hambriento[num_filosofos] = {0, 0, 0, 0, 0, 0, 0};

    while (true) {
        sleep_for(milliseconds(aleatorio<1, 5>()));
        esperando = 0;
        j = 0;
        // EL x2 Y /2 ES PARA HACER UN VECTOR DEL TAMAÑO JUSTO, PUES USAN SOLO
        // LOS PARES
        while (j < num_filosofos && !esperando) {
            if (fil_hambriento[j])
                MPI_Iprobe(j * 2, etiq_cocinero, MPI_COMM_WORLD, &esperando,
                           &estado);
            j++;
        }

        if (esperando) 
            id_aceptable = estado.MPI_SOURCE;
        else
            id_aceptable = MPI_ANY_SOURCE;


        MPI_Recv(&valor, 1, MPI_INT, id_aceptable, etiq_cocinero,
                 MPI_COMM_WORLD, &estado);
        id_filosofo = estado.MPI_SOURCE;

        cuenta[id_filosofo / 2]++;

        min = cuenta[0];
        for (int i = 0; i < num_filosofos; i++) {
            fil_hambriento[i] = false;
            if (cuenta[i] > max) max = cuenta[i];
            if (cuenta[i] < min) min = cuenta[i];
        }

        for (int i = 0; i < num_filosofos; i++) {
            if (cuenta[i] == min) fil_hambriento[i] = true;
        }

        cout << "Cocinero " << orden << " ha servido un plato al filósofo "
             << id_filosofo * 2 << ". El minimo es " << min
             << " y el maximo es " << max << "." << endl;
...
int main(int argc, char** argv) {
    ...
    if (num_procesos == num_procesos_actual) {
        ...
        if (id_propio == id_cocinero_uno)
            funcion_cocinero(0);
        else if (id_propio == id_cocinero_dos)
            funcion_cocinero(1);
        else if (id_propio % 2 == 0 && id_propio != num_filo_ten)  // si es par
